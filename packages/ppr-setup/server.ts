  ...
  const server = Bun.serve({
    ...
    async fetch(req: Request): Promise<Response> {
      ...
      if (isProd) {
        ...
        // SSR
        const entry = await import(path.join(root, 'dist', 'server', 'entry-server.js'))
        const render = entry.render || entry.default?.render || entry.default || entry
        const result = await render({ req, head: '' })
        if (result instanceof Response) return result
        if (typeof result === 'string') return new Response(result, { status: 200, headers: { 'content-type': 'text/html; charset=utf-8' } })
